...
from resources.schemas.Beer import (GetBeerSchema, MessageSchema,
                                    format_beer_data)
...
@app.get(
    "/get_beer/{name}",
    tags=["Beer"],
    response_model=GetBeerSchema,
    responses={
        200: {"model": GetBeerSchema},
...
def get_beer(name: str) -> Union[
    ...
    try:
        beer_data = TABLE_HANDLER.select_data_table(
            table=Beer, filter_select={Beer.name: name}
        )
